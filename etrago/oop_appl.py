...
if 'READTHEDOCS' not in os.environ:
    ...
    from etrago.components import Etrago_network
...
def etrago(args):
   ...
   scn_extension : NoneType or list
    ...
    """
    network = Etrago_network(args)
    ######## START: CAN BE MOVED TO Etrago.__init() ########
...
#        load_shedding(network)
    ...
    ######## END: CAN BE MOVED TO Etrago.__init() ########

    # ehv network clustering
    if args['network_clustering_ehv']:
        network = cluster_on_extra_high_voltage(network, with_time=True)

    # k-mean clustering
    if not args['network_clustering_kmeans'] == False:
        clustering = kmean_clustering(
                network,
                line_length_factor=1, # REMOVE TO args?
                remove_stubs=False, # REMOVE TO args?
                use_reduced_coordinates=False, # REMOVE TO args?
                bus_weight_tocsv=None,# REMOVE TO args?
                bus_weight_fromcsv=None,# REMOVE TO args?
                n_init=10,# REMOVE TO args?
                max_iter=100,# REMOVE TO args?
                tol=1e-6,# REMOVE TO args?
                n_jobs=-1)# REMOVE TO args?
    ...
    if 'network_preselection' in args['extendable']:
        extension_preselection(network, 'snapshot_clustering', 2)
    ...
    if args['pf_post_lopf']:
        network.pf_post_lopf(add_foreign_lopf=True,
                     q_allocation='p_nom',
                     calc_losses=True)
        #PART OF pf_post_lopf: calc_line_losses(network) 
        #PART OF pf_post_lopf: network = distribute_q(network, allocation='p_nom')

    # LOGS OF LOPF: if not args['extendable'] == []:
    # LOGS OF LOPF:    print_expansion_costs(network, args)
